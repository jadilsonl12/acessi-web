...
export default function BlogSection() {
  ...
  return (
    <section id="blog" className="py-20 px-4 relative overflow-hidden" aria-labelledby="blog-heading">
      <div className="absolute inset-0 bg-gradient-to-b from-gray-900 to-black" aria-hidden="true" />

      <div className="max-w-6xl mx-auto relative z-10">
        {/* Section Header */}
        <div className="text-center mb-16">
          <h2 id="blog-heading" className="text-4xl md:text-5xl font-bold text-white mb-4 font-sans">
        ...
        <div className="mb-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
            <div
              className="flex bg-white/5 backdrop-blur-md rounded-full p-1 border border-white/10"
              role="tablist"
              aria-label="Article view options"
            >
              <button
                onClick={() => setActiveTab("featured")}
                className={`flex items-center px-6 py-2 rounded-full transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-white/50 focus:ring-offset-2 focus:ring-offset-black ${
                  activeTab === "featured"
            ...
            {activeTab === "all" && (
              <div className="relative">
                ...
                <div id="search-help" className="sr-only">
                  Search through article titles and descriptions
                </div>
              </div>
            )}
          </div>
        ...
        <div role="tabpanel" id="blog-content" aria-labelledby={`${activeTab}-tab`}>
          {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" aria-label="Loading articles">
              {[...Array(6)].map((_, i) => (
                <div
                  key={i}
                  className="bg-white/5 backdrop-blur-md rounded-2xl p-6 animate-pulse border border-white/10"
                >
                  ...
                  {featuredPosts.map((post, index) => (
                    <article key={index}>
                      <a
                        ...
                        className="group bg-white/5 backdrop-blur-md rounded-2xl overflow-hidden border border-white/10 hover:bg-white/10 transition-all duration-300 hover:scale-105 hover:shadow-2xl block cursor-pointer focus:outline-none focus:ring-2 focus:ring-white/50 focus:ring-offset-2 focus:ring-offset-black"
                        aria-label={`Read article: ${cleanText(post.title, "Untitled Article")} - Opens in new tab`}
                      >
                    ...
                    filteredPosts.map((post, index) => (
                      <article key={index}>
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="group bg-white/5 backdrop-blur-md rounded-xl border border-white/10 hover:bg-white/10 transition-all duration-300 hover:scale-[1.02] p-6 block cursor-pointer focus:outline-none focus:ring-2 focus:ring-white/50 focus:ring-offset-2 focus:ring-offset-black"
                          aria-label={`Read article: ${cleanText(post.title, "Untitled Article")} - Opens in new tab`}
                        >
        ...
        {posts.length > 0 && activeTab === "featured" && (
          <div className="text-center mt-12">
            <button
              onClick={() => setActiveTab("all")}
              className="inline-flex items-center bg-white/5 backdrop-blur-md border border-white/10 text-white px-8 py-3 rounded-full hover:bg-white/15 transition-all duration-300 hover:scale-105 font-medium focus:outline-none focus:ring-2 focus:ring-white/50 focus:ring-offset-2 focus:ring-offset-black"
              aria-label={`View all ${posts.length} articles`}
            >
